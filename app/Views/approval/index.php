        ...
        <table>
            ...
            <tbody>
                <?php if (empty($requests)): ?>
                    <tr>
                        <td colspan="7" style="text-align:center; color:#9ca3af; padding: 2rem;">Tidak ada permintaan cuti yang perlu disetujui.</td>
                    </tr>
                <?php else: ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <div style="display: flex; gap: 0.5rem; flex-wrap: wrap;">
                                    <form action="<?= base_url('approval/process/' . $row['id'] . '/approved') ?>" method="post" style="display:inline;" id="form-approve-<?= $row['id'] ?>">
                                        <?= csrf_field() ?>
                                        <button type="button" class="btn btn-sm btn-primary" onclick="confirmApprove(<?= $row['id'] ?>)">Setuju</button>
                                    </form>
                                    
                                    <button type="button" class="btn btn-sm btn-warning" onclick="toggleNote(<?= $row['id'] ?>, 'deferred')" style="background-color: #f59e0b; color: white;">Tangguhkan</button>
                                    <button type="button" class="btn btn-sm btn-info" onclick="toggleNote(<?= $row['id'] ?>, 'changed')" style="background-color: #3b82f6; color: white;">Perubahan</button>
                                    <button type="button" class="btn btn-sm btn-danger" onclick="toggleNote(<?= $row['id'] ?>, 'rejected')">Tolak</button>
                                </div>
                                
                                <div id="note-form-<?= $row['id'] ?>" style="display:none; margin-top: 10px;">
                                    <form action="" method="post" id="form-note-<?= $row['id'] ?>">
                                        <?= csrf_field() ?>
                                        <textarea name="note" placeholder="Catatan/Alasan..." required style="width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 4px; margin-bottom: 0.5rem; font-family: sans-serif;"></textarea>
